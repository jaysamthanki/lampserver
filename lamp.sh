#!/bin/sh
# Centos 7.5 Webserver Setup
# Variables MYSQLPASSWORD HOSTNAME SERVERADMINEMAIL FQDN

echo "Techie Networks LAMP Server Build v1.01"
...
echo "$output" > /etc/yum.repos.d/mariadb.repo

# Install prereqs
echo "Installing Packages"
yum erase -y iwl100-firmware iwl105-firmware iwl135-firmware iwl2000-firmware iwl2030-firmware iwl3160-firmware iwl1000-firmware iwl3945-firmware iwl4965-firmware iwl5000-firmware iwl5150-firmware iwl6000-firmware iwl6000g2a-firmware iwl6000g2b-firmware  iwl6050-firmware iwl7260-firmware iwl7265-firmware
yum install -y httpd dos2unix mariadb-server php72w-fpm gcc git wget glibc-static net-tools mod_ssl psmisc fail2ban php72w-mysql php72w-devel php72w-pear php72w-gd php72w-mcrypt php72w-opcache php72w-mbstring certbot-apache
...
# ld2chroot
# ~~~~~~~~~
wget https://www.deferopro.com/ld2chroot.txt
mv -f ld2chroot.txt /usr/bin/ld2chroot
dos2unix /usr/bin/ld2chroot
chmod 755 /usr/bin/ld2chroot

# Update Webs
# ~~~~~~~~~~~
wget https://www.deferopro.com/updatewebs.txt
...
# Make a dummy SSL
/etc/ssl/certs/make-dummy-cert /etc/ssl/certs/$FQDN.crt
